...
def getContentIndex(currentText,currentQuery,docIndex,doc_id):
    ...
    if (currentText[-(len(currentQuery)+2):].find(currentQuery) != -1):
        CurText = preprocessor(currentText)
        CurQuery = preprocessor(currentQuery)
        print('****'+currentText[-(len(currentQuery)):])
        print('****'+currentQuery)


        if (CurText.find(CurQuery) != -1):
            print('####'+CurText[-(len(CurQuery)):])
            print('####'+CurQuery)
...
def getContentNounIndex(currentText,hou_halfText,currentQuery,docIndex,doc_id):
    print("currentText:"+currentText)
    print("hou_halfText:"+hou_halfText)
    print("currentQuery:"+currentQuery)
    print(docIndex)
    print(doc_id)
    print(len(currentText))
    ...
    if (currentText[-(len(currentQuery)+2):].find(currentQuery) != -1):
        ...
        if (CurText.find(CurQuery) != -1):
            ...
            if (curTextIndex.find(curQueryIndex) != -1):
                ...
                soup1 = BeautifulSoup(currentText, 'html.parser')
...
def readQueryFile(path):
    ...
    global doc_id_list
    global query_beg_list
    global query_end_list
    ...
    with open(path,encoding='utf-8') as f:
        ...
        while line:
            ...
            if(line.find('<beg>')!=-1):
                pos1 = line.find('<beg>')
                pos2 = line.find('</beg>')
                beg = line[pos1+5:pos2]
                query_beg_list.append(beg)
                # print(beg)
            if(line.find('<end>')!=-1):
                pos1 = line.find('<end>')
                pos2 = line.find('</end>')
                end = line[pos1+5:pos2]
                query_end_list.append(end)
                # print(end)
...
def findDocQueryAndDbAnswer(docPath,year,testFalg):
    ...
    with open(docPath,encoding='utf-8') as docF:
        ...
        while doc_line:
            ...
            if (doc_count > 0):
                ...
                with open(docP + "\\" + doc_id+'.txt' , encoding='utf-8') as textF:
                    docText = textF.read()
                    ...
                    if(doc_id_list[i]==doc_id):
                        ...
                        for j in range(len(answer_query_id_list)):
                            if(answer_query_id_list[j]==query_id_list[i]):
                                ...
                                currentText = docText[:int(query_end_list[i]) + 1].replace('\n', ' ')
                                hou_halfText = docText[int(query_end_list[i]) + 1:].replace('\n', ' ')
                                ...
                                contentNounIndex,_,_,_=getContentNounIndex(currentText,hou_halfText,currentQuery,docIndex,doc_id)
                                ...
                                tempString = doc_id_list[i] + ' ' + answer_query_id_list[j] + ' ' + answer_query_name_list[j] +' '+answer_db_id_list[j]+' '+answer_db_name_list[j]+' beg='+query_beg_list[i]+' end='+query_end_list[i]
                                ...
                                queryTextIndexFile.write(query_id_list[i]+' '+getWordIndex(newText).strip(' ') + '\n')
...
if __name__=='__main__':
    # readWordVecIndex()
    # docPath=u'DocFileName.txt'
    # queryPath=u'H:\\yaojuan\\QUERY\\2014\\eval\\tac_kbp_2014_english_EDL_evaluation_queries.xml'
    # answerPath=u'newAnswer.txt'
    # dbIdNamePath=u'DBIdName.txt'
    #
    # readIdNameFile(dbIdNamePath)
    # readQueryFile(queryPath)
    # readAnswerFile(answerPath)
    # findDocQueryAndDbAnswer(docPath,queryPath)

    year = '2014'  ####2012 2013 2014  trian2014
    testFlag = False
    readWordVecIndex()
    if(testFlag):
        ...
        queryPath = u'H:\yaojuan\QUERY\\' + year + '\\training\\tac_kbp_' + year + '_english_EDL_training_queries.xml'
